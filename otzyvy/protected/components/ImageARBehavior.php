	...
	protected function createProcessor($klass, $srcPath) {
		...
		if (! isset($this->_img_config)) {
			if (isset($this->useImageMagick)) {
				$params = array();
				if (is_string($this->useImageMagick)) $params['directory'] = $this->useImageMagick;
				$this->_img_config = array('driver' => 'ImageMagick', 'params' => $params);
			} else {
				$this->_img_config = array('driver' => 'GD', 'params' => array());
			}
		}
